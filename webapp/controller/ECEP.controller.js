sap.ui.define([
	...
	return Controller.extend("gitTest.GitTest.controller.ECEP", {
		...
		onPerPersonalRefresh: function () {
			...
			var dataMode11 = this.getOwnerComponent().getModel("TEST_SCHEMA");
			dataMode11.read(
				"/USERINFO", {
		...
		onDeleteBackground_Test4ECEP: function () {
			...
			for (var index in selectedIndexList) {
				var deleteModel = {};
				deleteModel.userId = listTable.getCellControl(selectedIndexList[index], 1, false).getText();
				deleteModel.backgroundElementId = listTable.getCellControl(selectedIndexList[index], 2, false).getText();
												...
												success: function (data1) {
													...
													for (var k = 0; k < fromPerPersonalModellList.length; k++) {
														...
														for (var l = 0; l < results.length; l++) {
															if (results[l].fromTableName == "PerPersonal" && results[l].colValue1 == fromPerPersonalModel.startDate) {
														...
														if (upsertBackground_Test4ECEPModel.backgroundElementId) {
															upsertBackground_Test4ECEPModel.__metadata = {
																"uri": "Background_Test4ECEP(userId='" + upsertBackground_Test4ECEPModel.userId + "', backgroundElementId=" +
																	upsertBackground_Test4ECEPModel.backgroundElementId + ")",
											...
											"/Background_Test4ECEP", {
												...
												success: function (data1) {
													...
													for (var k = 0; k < fromCust_testMdfWithHistoryModellList.length; k++) {
														...
														//if already exists
														for (var l = 0; l < results.length; l++) {
															if (results[l].fromTableName == "cust_testMdfWithHistory" && results[l].colValue1 ==
																fromCust_testMdfWithHistoryModel.effectiveStartDate) {
																upsertBackground_Test4ECEPModel.backgroundElementId = results[l].backgroundElementId;
																break;
															}
														}
		...
		addBatchOperation: function (upsertBackground_Test4ECEPModelList, deleteBackground_Test4ECEPModelList, upsertTestTimeModel) {
			...
			if (deleteBackground_Test4ECEPModelList && deleteBackground_Test4ECEPModelList.length > 0) {
				...
				for (var index in deleteBackground_Test4ECEPModelList) {
					dataModel.remove(
						"/Background_Test4ECEP(userId='" + deleteBackground_Test4ECEPModelList[index].userId + "', backgroundElementId=" +
						deleteBackground_Test4ECEPModelList[index].backgroundElementId + ")", {
							groupId: "updateGroup"
						}
					);
				}
